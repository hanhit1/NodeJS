...
const years = containNumber.match(/\d{4}/g).map(Number);
const result = [];
for (const year of years) {
  result[Math.floor(year / 10) * 10 ] = (result[Math.floor(year / 10)* 10] || 0) + 1;
}
console.log(years);
let max = 0;
let maxCount = 0;
...
console.log(`Thập niên ${maxCount % 100} thế kỷ ${Math.floor(maxCount / 100) + 1} có nhiều năm nhất với ${max} năm`);